          ...
          <div className="rounded-[2rem] border border-slate-200 bg-white p-8 shadow-sm dark:border-slate-800 dark:bg-slate-950">
            ...
            <div className="mt-6 flex items-center gap-4">
              <div className="flex h-12 w-12 items-center justify-center rounded-full bg-sky-100 font-semibold text-sky-700 dark:bg-slate-900 dark:text-sky-300">
                {(task.client?.name || "C").charAt(0)}
              </div>
              <div>
                <p className="font-semibold text-slate-950 dark:text-white">{task.client?.name || "Unknown client"}</p>
                <p className="text-sm text-slate-500 dark:text-slate-400">{task.client?.email || task.client_email || "Contact available after application"}</p>
              </div>
            </div>
