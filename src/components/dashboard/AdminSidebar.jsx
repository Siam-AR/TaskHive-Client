...
export default function AdminSidebar({ user }) {
  ...
  const sidebarContent = (
    <div className="flex h-full flex-col gap-4">
      ...
      <div className="mt-auto rounded-[1.5rem] border border-slate-200 bg-white/80 p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900/80">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-base font-semibold text-slate-950 dark:text-white">{user?.name || "Admin"}</h2>
            <p className="text-sm capitalize text-slate-600 dark:text-slate-400">{user?.role || "admin"}</p>
          </div>
        </div>
